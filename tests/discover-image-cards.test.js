...
test('发现卡使用左右画报分栏、大日期、真实发起人与可选说明', () => {
  ...
  assert.match(style, /\.activity-description\s*\{[^}]*padding:\s*8rpx 14rpx;[^}]*background:\s*#f8fafc;[^}]*border:\s*1rpx solid #edf1f5;/s);
  const pageStyle = read('pages/discover/index.wxss');
  assert.match(pageStyle, /\.skeleton-card\s*\{[^}]*min-height:\s*390rpx;[^}]*padding:\s*0;[^}]*box-shadow:\s*0 6rpx 20rpx/s);
  assert.match(pageStyle, /\.skeleton-cover\s*\{[^}]*flex:\s*0 0 46%;[^}]*min-height:\s*390rpx;/s);
