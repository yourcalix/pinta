...
test('新首页按问候、Lifestyle Hero、三快捷卡、搜索和活动顺序排版', () => {
  ...
  assert.match(style, /\.hero-illustration-slot\s*\{[^}]*right:\s*0;[^}]*bottom:\s*-94rpx;[^}]*width:\s*430rpx;[^}]*height:\s*860rpx;/s);
  assert.match(style, /\.hero-illustration-image\s*\{[^}]*-webkit-mask-image:\s*linear-gradient\(to bottom, #000 0%, #000 77%, rgba\(0, 0, 0, 0\.72\) 86%, transparent 100%\);/s);
  assert.match(style, /@media \(max-width:\s*340px\)[\s\S]*\.hero-illustration-slot\s*\{[^}]*bottom:\s*-86rpx;[^}]*width:\s*373rpx;[^}]*height:\s*746rpx;/s);
