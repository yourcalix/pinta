...
test('司机意向不授予权限，只有开发审核通过才物化司机与车辆事实源', async () => {
  const store = new MemoryStore({ users: [activeUser('candidate'), activeUser('reviewer', 'admin')] });
...
test('上传凭据严格绑定本人和资料类型，司机申请业务幂等可恢复', async () => {
  const store = new MemoryStore({ users: [activeUser('candidate'), activeUser('attacker')] });
  const service = createPinbaService({
