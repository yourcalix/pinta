...
function presentation(activity) {
  ...
  else if (activity.viewerRole === 'owner' && activity.status === 'RECRUITING') { primaryAction = 'manage'; primaryLabel = '管理拼单与申请'; }
  ...
  else if (activity.canApply) { primaryAction = 'apply'; primaryLabel = '申请加入'; }
  else if (activity.status === 'RECRUITING' && activity.viewerApplication && activity.viewerApplication.status === 'PENDING') primaryLabel = '加入申请审核中';
  else if (activity.status === 'RECRUITING' && activity.viewerRole === 'member') primaryLabel = '已加入 · 等待成团';
  ...
  return {
    ...
    detailSlots: slots.slice(0, 5).map((slot, index) => ({ ...slot, layer: 5 - index })),
    hiddenMembers: slots.slice(5).filter(slot => !slot.empty).length,
...
Page({
  data: { id: '', activity: null, detailRows: [], loading: true, error: '', errorCode: '', applying: false, note: '', showApply: false,
    contentTopInset: 88, navTop: 36, singlePage: true, coverSrc: '', coverFailed: false, detailSlots: [], hiddenMembers: 0, primaryAction: '', primaryLabel: '', opening: false },
  onLoad(options = {}) {
  ...
  onUnload() { this._disposed = true; this._visible = false; this._loadSeq = (this._loadSeq || 0) + 1; },
  handleBack() {
    let pages = [];
