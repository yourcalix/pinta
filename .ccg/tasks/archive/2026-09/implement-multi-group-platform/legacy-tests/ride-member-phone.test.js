...
function setup() {
  const store = new MemoryStore({
    ...
    studentVerifications: ['owner', 'member'].map((userId) => ({ id: userId, userId, status: 'APPROVED', revision: 1, updatedAt: NOW.toISOString() })),
    drivers: [
...
test('只有当前承接司机可读取全部 ACTIVE 成员电话，取消承接后立即失权', async () => {
  ...
  assert.equal(owner.error.code, 'DRIVER_NOT_APPROVED');
