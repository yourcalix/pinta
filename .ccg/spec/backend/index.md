...
- 用户关注关系使用独立 `profileFollows` 集合与“关注者 + 目标用户”派生的确定性文档 ID；关注写入只接受绑定当前查看者的短期 `profileNavToken` 与目标布尔状态，严禁客户端提交 userId/openid/targetId。关系状态和双方 `followingCount / followerCount` 必须在同一事务更新，重复关注或重复取消的计数增量必须为 0。本人可通过 `profile.follow.list` 查看自己的关注/粉丝成员；列表使用绑定查看者、Tab 与排序锚点的随机不透明游标，成员 DTO 不得输出 userId/openid/关系文档 ID，只签发短期 `socialProfileNa_` 主页凭据。列表不提供私信、在线状态、移除粉丝或拉黑能力。
- 讨论动态使用独立 `communityActivities` 收件箱集合：他人回复在回复事务内写入单条事件；帖子点赞按“收件人 + 帖子”、评论点赞按“收件人 + 评论”分别聚合，并在点赞事务内更新。自回复、自赞不生成动态；取消点赞归零后聚合动态转为 `INACTIVE`。“收到的赞”必须同时合并 `POST_LIKED | REPLY_LIKED`，Cloud 查询按等值类型分别取流后稳定合并，不使用 `or` 绕过复合索引。聚合动态使用可变 `updatedAt` 让新赞重新上浮，因此翻页采用实时流的最终一致语义而非跨请求快照隔离；刷新首屏必须收敛最新顺序。未读汇总必须由服务端覆盖近 30 天全部 `ACTIVE + read:false` 活动并按 `ALL / REPLIES / LIKES` 返回，不得由首屏分页推算；标记已读必须绑定收件人、对应帖子与调用方看到的 `updatedAt` 版本，并在事务内比较，旧阅读请求不得清除随后聚合的新赞。列表只返回当前调用者近 30 天数据，游标必须绑定筛选 Tab；评论赞的原帖或目标评论删除/下架后只返回受控失效态且隐藏两段正文，不得泄露内部用户 ID。
- 社区定向回复只允许可选 `replyToId` 指向同一帖子内仍为 `ACTIVE` 的单层回复，归属与状态必须在回复写事务内重验。普通回复通知帖子作者，定向回复通知目标回复作者，自回复不生成动态；公开 DTO 只输出目标昵称与 `ACTIVE | UNAVAILABLE` 状态，不得暴露目标回复 ID 或内部用户 ID。新写入遇到失效目标必须拒绝，已存在回复在目标后来删除时只做受控失效展示。
- 默认使用 CommonJS 和 Node.js 18 兼容语法，避免本机 npm 版本不匹配带来的构建依赖。

## 身份与权限

- 新用户一次性欢迎权益必须在服务端首次创建账号时写入稳定的 campaign/variant 与 `PENDING | SEEN` 状态，不能由客户端每次随机，也不能用资料完整度猜测。旧账号缺少该字段时视为不参与且不得批量补弹；登录只输出 campaign、pending 和有限 variant 枚举，确认动作必须鉴权、幂等并在用户文档事务中消费。
...
- 单个用户在单个活动中的申请与成员身份均使用 `activityId + userId` 派生的确定性文档 ID；详情查看者身份必须按 `doc(id)` 精确读取并复核文档归属与有效状态，不得退回需要复合索引的多字段查询，以免登录后的公开详情被索引配置故障击穿。
...
- 搭子星球球面目录与在线 Presence 必须分离：`companion.directory.snapshot` 从 `ACTIVE` 用户事实中按 `createdAt desc + _id desc` 最多返回最近加入的 50 个脱敏节点，不查询或公开目录总数；首次登录创建账号后即进入目录。快照同时返回 Presence `onlineTotal`、`serverNow` 与绑定查看者视图的 ETag；ETag 命中时返回 `unchanged: true` 并省略 `users`，但仍须返回最新在线总数。资料未完成时只输出通用昵称和空白公开资料，不补造字段。目录 DTO 只允许短时 `displayToken`、不可逆的稳定 `renderKey`、昵称、布局种子和本人标识，不得包含 userId、openid、头像、位置、联系方式或稳定主页凭据。目录节点令牌按五分钟窗口轮换，点击时才由受保护的 `companion.directory.profile.nav.create` 重新解析目标并签发绑定访问者的 60 秒随机票据；公开主页再次复核票据、目标账号状态，且目录来源固定 `online: false`，不得借目录存在推断在线。
...
- 拼享惠使用独立 `benefit` 契约，`typeData` 只允许商家/平台、优惠类型、门槛、目标价格、预计节省、履约方式和优惠详情白名单，不得存储手机号、微信号、二维码或饭桌字段。`ONLINE` 必须在服务端和 Mock 强制丢弃客户端残留的 `meetingPoint`，以商家/平台作为 `placeLabel` 且不进入 nearby；`OFFLINE` 必须要求 AMAP/GCJ-02 公开会合点，以地点 label 作为 `placeLabel` 并允许进入 nearby。Cloud、Mock、前端 payload 和公开 DTO 必须保持该语义一致，公开结果只允许地点 label/address，不返回精确坐标。
...
- 新增业务集合上线前，必须通过权威集合列表或真实只读请求验证目标环境中的集合事实；单集合存在性辅助检查不能作为唯一发布凭据。
- 默认 Mock 的输入校验、规范化结果、长度边界与用户可见错误文案必须和正式云函数契约一致；选填字段不得只在 Mock 中被升级为必填。
- 资料更新中的新增选填字段必须区分兼容语义：字段缺省或值为 `undefined` 表示旧客户端未修改并保留旧值，`null` 或约定的空字符串才表示用户主动清空；正式服务、Mock 与测试必须保持一致。
- 必测：状态转换、幂等、重复申请、最后名额并发、越权、联系信息隔离、取消/退出、内容拒绝。
- 对 SDK 限制增加静态回归检查，例如禁止 CloudBase 事务回调中出现 `where()`。
- 涉及“允许文档不存在”的 CloudBase 事务测试替身必须模拟真实 SDK：`doc(id).get()` 对缺失文档抛出 `-502005`，不得统一返回 `{ data: null }` 掩盖 Store 边界遗漏。
