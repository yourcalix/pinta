...
- 讨论动态使用独立 `communityActivities` 收件箱集合：他人回复在回复事务内写入单条事件；帖子点赞按“收件人 + 帖子”、评论点赞按“收件人 + 评论”分别聚合，并在点赞事务内更新。自回复、自赞不生成动态；取消点赞归零后聚合动态转为 `INACTIVE`。“收到的赞”必须同时合并 `POST_LIKED | REPLY_LIKED`，Cloud 查询按等值类型分别取流后稳定合并，不使用 `or` 绕过复合索引。聚合动态使用可变 `updatedAt` 让新赞重新上浮，因此翻页采用实时流的最终一致语义而非跨请求快照隔离；刷新首屏必须收敛最新顺序。列表只返回当前调用者近 30 天数据，游标必须绑定筛选 Tab；评论赞的原帖或目标评论删除/下架后只返回受控失效态且隐藏两段正文，不得泄露内部用户 ID。
