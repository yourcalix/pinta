...
- 讨论动态使用独立 `communityActivities` 收件箱集合：他人回复在回复事务内写入单条事件，帖子点赞按“收件人 + 帖子”聚合并在点赞事务内更新；自回复、自赞不生成动态。列表只返回当前调用者近 30 天数据，游标必须绑定筛选 Tab，原帖删除或下架后只返回受控失效态，不得泄露正文或内部用户 ID。
