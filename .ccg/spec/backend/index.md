...
- 用户关注关系使用独立 `profileFollows` 集合与“关注者 + 目标用户”派生的确定性文档 ID；关注写入只接受绑定当前查看者的短期 `profileNavToken` 与目标布尔状态，严禁客户端提交 userId/openid/targetId。关系状态和双方 `followingCount / followerCount` 必须在同一事务更新，重复关注或重复取消的计数增量必须为 0。公开 DTO 只返回计数和当前查看者关系状态；一期不提供关系列表、通知、私信或在线状态。
