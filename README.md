...
8. 部署每日敏感资料清理任务：删除已过期的司机资料与头像 `PREPARED` staging 文件，并在 `retentionUntil` 到期后删除 `RETENTION_PENDING` 的 sealed 文件与 `driverSecrets`；清理动作必须写审计且可幂等重试。
...
- `activityGroupMessages`：`activityId(升序) + sequence(降序)`；消息序号由活动事务递增，集合禁止客户端直接读写。
- `activityGroupReadStates`：使用活动与用户派生的确定性文档 ID，无需额外索引；只记录当前成员周期内已实际取得的消息序号。

首次上线真实群聊前必须执行一次受控数据迁移：在确认 `activityGroupMessages` 尚无历史数据的前提下，为既有活动初始化 `groupSequence: 0`，并为当时仍为 `ACTIVE` 的成员写入 `groupWindow: { generation: 1, after: 0 }`。运行时代码对缺失或损坏的边界一律拒绝访问，不会默认从 0 放开历史；迁移脚本和 CloudBase 索引需先在测试环境核验，本仓库本轮不自动部署或改写线上数据。
...
- 活动详情“群聊”只面向当前 `ACTIVE` 成员。每次加入建立新的 `groupWindow`，只可读取序号严格大于本次加入边界的消息；退出立即失去读写权限，重新加入不会恢复之前任何周期的历史。
- 招募中、已成团和进行中的当前成员可发送最多 500 字纯文字；完成、取消或过期后仍为当前成员者只读；下架活动完全不可访问。
- “聊下”由服务端只根据 `activityId` 派生发起人，客户端不能指定目标用户。咨询不要求先加入活动，但只在招募中、已成团和进行中可发送；终态保留双方历史只读，下架后不返回会话。
- 群聊与咨询继续使用内容安全、联系方式/外链拒绝、稳定客户端消息 ID、低频轮询和离屏响应隔离。真实环境至少用发起人、成员、未加入者三个账号验证审批前后、退出/重入、终态、下架、弱网重试和输入法行为。
